...
This package ships a free, CDN-hosted snapshot of the data — that's still the right choice when the app must work offline-first, a periodic snapshot is enough, or you just need simple lookups/dropdowns.
...
**Start free with 3,000 requests per month.** [Compare plans](https://countrystatecity.in/pricing?source=npm&campaign=sdk_api_migration&package=countries-browser) or [get an API key](https://app.countrystatecity.in?source=npm&campaign=sdk_api_migration&package=countries-browser).
