...
describe('search commands', () => {
  ...
  describe('--fields/--sort passthrough', () => {
    ...
    it('does not crash when --fields omits name while --filter is present', async () => {
      vi.mocked(get).mockResolvedValue({ data: [{ id: 101, iso2: 'IN' }], usage: null });
      vi.spyOn(console, 'log').mockImplementation(() => {});

      await expect(
        program.parseAsync(['node', 'csc', 'search', 'countries', '--filter', 'india', '--fields', 'iso2'])
