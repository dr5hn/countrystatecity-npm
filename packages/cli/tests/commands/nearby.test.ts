...
import { call } from '../../src/lib/api.js';
...
describe('nearby command', () => {
  ...
  it('passes parsed, typed options through to client.search.nearby', async () => {
    ...
    vi.mocked(call).mockImplementation(async (fn) => {
      const fakeClient = {
        search: {
          nearby: vi.fn(async (args: unknown) => {
            captured = args;
            return { data: [MUMBAI_NEARBY], meta: { retryCount: 0 } };
          }),
        },
      };
    ...
    await program.parseAsync([
      ...
      '--type', 'city', '--country', 'in', '--radius', '25', '--limit', '10', '--min-population', '100000',
    ]);
  ...
  it('defaults type to city, radius to 25, and limit to 20', async () => {
    let captured: unknown;
    vi.mocked(call).mockImplementation(async (fn) => {
      const fakeClient = { search: { nearby: vi.fn(async (args: unknown) => { captured = args; return { data: [], meta: { retryCount: 0 } }; }) } };
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      return fn(fakeClient as any);
    });
  ...
  it('renders a table with a Distance (km) column', async () => {
    vi.mocked(call).mockResolvedValue({ data: [MUMBAI_NEARBY], usage: null });
  ...
  it('shows a friendly message when there are no nearby results', async () => {
    vi.mocked(call).mockResolvedValue({ data: [], usage: null });
