...
**Start free with 3,000 requests per month.** [Compare plans](https://countrystatecity.in/pricing?source=npm&campaign=sdk_api_migration&package=currencies) or [get an API key](https://app.countrystatecity.in?source=npm&campaign=sdk_api_migration&package=currencies).
...
```typescript
// Before
import { getCurrencyByCode } from '@countrystatecity/currencies';
...
const { data: usd } = await csc.currencies.get('USD');
```
